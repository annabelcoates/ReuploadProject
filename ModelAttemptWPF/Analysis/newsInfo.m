...
for i = 1:nRuns
    for j = 1:varParamVals_len
        ...
        resultsPaths{i, j} = fullfile(tempPath{j}, 'newsInfo.csv');
    end
end
...
for varIdx = 1:varParamVals_len
    ...
    newsInfoStruct.(varParam) = zeros(nRuns, csvInputDims(1), csvInputDims(2));
    newsInfoStruct.(varParam)(1,:,:) = csvInput;

    for i=2:nRuns
        fileName = resultsPaths{i, varIdx};
        csvInput=csvread(fileName,1,1);
        newsInfoStruct.(varParam)(i,:,:) = csvInput;
    end
end
%%
