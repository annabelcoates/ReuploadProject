...
for i = 1:nRuns
    for j = 1:varParamVals_len
        ...
        resultsPaths{i, j} = fullfile(tempPath{j}, 'trueShareProbs.csv');
    end
end
...
for varIdx = 1:varParamVals_len
    ...
    trueShareProbsStruct.(varParam) = zeros(nRuns, csvInputDims(1), csvInputDims(2));
    trueShareProbsStruct.(varParam)(1,:,:) = csvInput;

    for i=2:nRuns
        fileName = resultsPaths{i, varIdx};
        csvInput=csvread(fileName,1,1);
        trueShareProbsStruct.(varParam)(i,:,:) = csvInput;
    end
end
%%
