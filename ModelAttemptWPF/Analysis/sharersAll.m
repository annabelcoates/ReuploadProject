...
for i = 1:nRuns
    for j = 1:varParamVals_len
        ...
        resultsPaths{i, j} = fullfile(tempPath{j}, 'sharersAll.csv');
    end
end
...
for varIdx = 1:varParamVals_len
    ...
    sharersAllStruct.(varParam) = zeros(nRuns, csvInputDims(1), csvInputDims(2));
    sharersAllStruct.(varParam)(1,:,:) = csvInput;

    for i=2:nRuns
        fileName = resultsPaths{i, varIdx};
        csvInput=csvread(fileName,1,1);
        sharersAllStruct.(varParam)(i,:,:) = csvInput;
    end
    sharersAllStruct.extra.timeOfRun = timeOfRun;
    sharersAllStruct.extra.varParamVals = varParamVals;
    sharersAllStruct.extra.varParamVals_len = varParamVals_len;
end
%%
