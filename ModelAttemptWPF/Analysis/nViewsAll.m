...
for i = 1:nRuns
    for j = 1:varParamVals_len
        ...
        resultsPaths{i, j} = fullfile(tempPath{j}, 'nViewsAll.csv');
    end
end
...
for varIdx = 1:varParamVals_len
    ...
    nViewsAllStruct.(varParam) = zeros(nRuns, csvInputDims(1), csvInputDims(2))
    nViewsAllStruct.(varParam)(1) = csvInput

    for i=2:nRuns
        fileName = resultsPaths{i, varIdx};
        csvInput=csvread(fileName,1,1);
        nViewsAllStruct.(varParam)(i,:,:) = csvInput
    endscriptPath = fileparts(mfilename('fullpath'));
    cd (scriptPath);
    timesList = loadTimes(scriptPath);
    lastTime = size(timesList);
    lastTime = lastTime(1);
