...
function beliefEmoteSpreadNew(s,v,newsInfo,nF,nT, twoD, threeD)
    view(3);
    n = nT + nF;
    ...
    for i = 1:s.extra.varParamVals_len
        ...
        for j = 1:runs    
            % OLD newsProps = sum(newsInfo.(inputs{i}),1)./runs;
            newsProps = newsInfo.(inputs{i});
            newsProps = newsProps(j, :, :);
            ...
            % OLD mata = sum(s.(inputs{i}),1)./runs;
            mata = s.(inputs{i});
            mata = mata(j, :, :)
            mata = squeeze(mata);
            ta = mata(:, size(mata, 2));
            ...
            %% Views
            % OLD matb = sum(v.(inputs{i}),1)./runs;
            matb = v.(inputs{i});
            matb = matb(j, :, :)
            matb = squeeze(matb);
            tb = matb(:, size(matb, 2));
