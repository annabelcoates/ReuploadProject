...
for a=1:length(values)
    ...
    for i=1:nRuns
        sFilename=['..\Results\' filename '_' int2str(i) '\nSharesAll.csv'];
        vFilename=['..\Results\' filename '_' int2str(i) '\nViewsAll.csv'];
        sharesData=csvread(sFilename);
        viewsData=csvread(vFilename);
        fakeShares(i,:)=mean(sharesData(1:nFake,1:1000));
        fakeViews(i,:)= mean(viewsData(1:nFake,1:1000));
        trueShares(i,:)=mean(sharesData(nFake+1:(nTrue+nFake),1:1000));
        trueViews(i,:)=mean(viewsData(nFake+1:(nTrue+nFake),1:1000));
    end
